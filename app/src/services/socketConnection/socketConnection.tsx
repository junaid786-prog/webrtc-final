...
class SocketConnection {
    ...
    initializePeersEvents = () => {
        this.myPeer.on('connection', (conn) => {
            console.log('connection establised', conn)
        })
        this.myPeer.on('open', async (id) => {
            ...
            setTimeout(() => this.socket.emit('join-room', userData), 3000)
            this.socket.emit('all_users');
        });
    ...
    setNavigatorToStream = async () => {
        ...
        if (stream) {
            ...
            //setTimeout(this.newUserConnection, 3000, stream)
        }
    }
    ...
    setPeersListeners = (stream: MediaStream) => {
        this.myPeer.on('call', (call: any) => {
            ...
            call.on('stream', (userVideoStream: MediaStream) => {
                console.log('remote user stream', userVideoStream);
    ...
    newUserConnection = (stream: MediaStream) => {
        this.socket.on('new-user-connect', (userData: any) => {
            ...
            //setTimeout(this.connectToNewUser, 3000, userData, stream)
    ...
    connectToNewUser(userData: any, stream: MediaStream) {
        ...
        console.log('called by stream ------', stream)
        this.myPeer.connect(userID)
